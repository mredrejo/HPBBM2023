...
## Data matrix format (wide vs. long table)
...
Using *coli_genomes_renamed.csv*, could you make a table with genome frequencies by isolation source (*Source*), phylogenetic group (*Phylogroup*) and multilocus sequence type (*Sequence.Type*)?
...
The applications of *apply* functions are very wide. You can use them to parse data with many different input and output structures, even for plots. Check out the following example.

```{r}
#how can you generate several plots for different variable by factors??
par(mfrow=c(1,3)) #arrange the three plots in a row (see Lesson 11)

#option 1: looping
system.time(for (i in 9:11){
  barplot(by(coli_genomes[,i],list(coli_genomes$Source),mean),legend.text = names(coli_genomes[i]))
})
#option2: lapply
ploteando <- function(x) {barplot(by(coli_genomes[,x],list(coli_genomes$Source),mean),legend.text = names(coli_genomes[x]))}
system.time(lapply(9:11,FUN=ploteando))
```

In this example `lapply()` returns a list of plots and even for only 3 plots you can notice that it is faster than a loop.
