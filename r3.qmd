...
::: callout-caution
Note that if you save your data with `save()`, it cannot be restored under a different name. The original object names are automatically used.
If you want to save the object and restore it with a different name you need to use the functions `saveRDS()` and `readRDS()`
:::
