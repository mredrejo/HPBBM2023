...
Data visualizations are a vital component of a data analysis, as they have the capability of summarizing large amounts of data efficiently in a graphical format and bring out new insights that can be difficult to understand from raw data. Moreover, from the perspective of the researcher or data analyst, plotting the datasets will help to familiarize with the data and plan the next steps in your analysis. Also, an early plot can help you to identify statistics pitfalls.
...
This well-known quartet highlights the importance of graphing data prior to analysis, and why statistical reviewers often ask for such graphs to be made available. Check out also the Dinosaurus example in the [reference 2 below](#refs).

## Which plot should I do with my data?

There are many chart types available, each with its own strengths and use cases. One of the trickiest parts of the analysis process is choosing the right way to represent your data using one of these visualizations. A wrong plot can lead to confusion or data misinterpretation (see some examples of bad graphs [here](https://www.analyticsvidhya.com/blog/2021/02/data-visualization-bad-representation-of-data/)).
...
As a guidance, you can use the below *Chartchooser*, by Andrew Abela.
