...
Try to read again the file coli_genomes_renamed.csv that we saved in the previous lesson and save it ready for MS Excel.
...
```
Sometimes, you can come into two different namesake functions from independent packages. Also, to reduce the memory load you may want to use a function without loading the package. In those cases, there is a trick to call a specific function. In the example below, you will see that `ggplot()` function can be used if you specify the package with the prefix `ggplot::` or load the package in advance.
```{r error=TRUE}
